...
export default function PengajuanTelat() {
  ...
  const [estimated_arrival, setEstimated_arrival] = useState('');
  ...
  const handleUpdateStatus = async (id: string, newStatus: string, empId: string, date: string) => {
    const { error } = await supabase.from('late_requests').update({ status: newStatus }).eq('id', id);
    if (!error) {
      ...
      if (newStatus === 'Disetujui') {
        ...
        if (check.data) {
          await supabase.from('attendance').update({ status: 'Telat', notes: 'Disetujui sistem' }).eq('id', check.data.id);
        } else {
  ...
  const handleDelete = async (id: string, empId: string, date: string) => {
    ...
    const { error } = await supabase.from('late_requests').delete().eq('id', id);
    if (!error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-800">Pengajuan Telat</h1>
          <p className="text-sm text-slate-500">Kelola pengajuan dan konfirmasi keterlambatan hadir.</p>
        </div>
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle>Form Pengajuan Telat</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">Estimasi Kedatangan</label>
                <input required type="time" value={estimated_arrival} onChange={e => setEstimated_arrival(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm transition-all focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20" />
              </div>
