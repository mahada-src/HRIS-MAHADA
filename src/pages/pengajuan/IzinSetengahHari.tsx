...
export default function PengajuanIzinSetengahHari() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('half_day_requests').insert([{
  ...
  const handleUpdateStatus = async (id: string, newStatus: string, empId: string, date: string) => {
    const { error } = await supabase.from('half_day_requests').update({ status: newStatus }).eq('id', id);
    if (!error) {
      ...
      if (newStatus === 'Disetujui') {
        ...
        if (check.data) {
          await supabase.from('attendance').update({ status: 'Setengah Hari', notes: 'Disetujui sistem' }).eq('id', check.data.id);
        } else {
  ...
  const handleDelete = async (id: string, empId: string, date: string) => {
    ...
    const { error } = await supabase.from('half_day_requests').delete().eq('id', id);
    if (!error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-800">Izin 1/2</h1>
          <p className="text-sm text-slate-500">Kelola pengajuan izin meninggalkan pekerjaan di pertengahan jam kerja.</p>
        </div>
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle>Form Izin 1/2</CardTitle>
          </CardHeader>
          <CardContent>
