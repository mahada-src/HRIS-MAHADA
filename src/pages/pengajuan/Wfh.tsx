...
export default function PengajuanWfh() {
  ...
  const handleUpdateStatus = async (id: string, newStatus: string, empId: string, date: string) => {
    const { error } = await supabase.from('wfh_requests').update({ status: newStatus }).eq('id', id);
    if (!error) {
      ...
      if (newStatus === 'Disetujui') {
        ...
        if (check.data) {
          await supabase.from('attendance').update({ status: 'WFH', notes: 'Disetujui sistem' }).eq('id', check.data.id);
        } else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-800">Pengajuan WFH</h1>
          <p className="text-sm text-slate-500">Kelola pengajuan bekerja dari rumah (Work From Home).</p>
        </div>
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle>Form Pengajuan WFH</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">To-Do List Pekerjaan</label>
                <textarea required rows={3} value={todo_list} onChange={e => setTodo_list(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm transition-all focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"></textarea>
              </div>
